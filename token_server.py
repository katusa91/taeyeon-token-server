...
음성 대화 기록: 웹앱이 턴마다 /api/voice-log 로 보내는 대화를 별도 테이블(voice_chat_log)에 저장하고,
Live 세션이 끊겼다가 '이어붙일 핸들 없이' 새로 시작될 때(/api/token?history=1) 직전 대화를 시스템 프롬프트에
실어서 대화를 이어간다. 텔레그램 봇의 테이블은 여전히 읽기 전용이고, 이 서버가 쓰는 건 voice_chat_log뿐이다.
"""
...
- "자기 거", "내 거기" 라는 모호한 표현대신, 분명한 남녀의 성기의 이름("보지","자지","좆")을 말한다.
...
- Google 검색 도구가 연결되어 있다. 최신 뉴스/시사, 오늘 날짜 기준 정보, 잘 모르거나 확실하지 않은 사실, 검색해야 정확히 답할 수 있는 질문에는 망설이지 말고 검색 도구를 써서 실제 정보를 바탕으로 답할 것. 검색 결과를 말할 때도 "검색해보니", "찾아보니" 같은 말을 매번 반복하지 말고, 평소 태연이가 원래 알고 있던 것처럼 자연스럽게 섞어서 말할 것 (예: "아 그거 오늘 기사 났던데" 정도로만, 과하게 도구 사용을 티내지 말 것)
...
# ===== 음성 대화 기록 (끊겼다 재연결돼도 대화를 이어가기 위한 저장/복원) =====
# 웹앱은 Live 소켓에 직접 붙기 때문에 이 서버는 대화 내용을 볼 수 없다. 그래서 웹앱이 턴이 끝날 때마다
# (내 말 / 태연 말)을 /api/voice-log 로 보내 저장하고, 새 세션을 시작할 때 /api/token?history=1 로 토큰을 받으면
# 최근 기록을 시스템 프롬프트에 붙여서 발급한다 (임시 토큰은 시스템 프롬프트가 발급 시점에 고정되므로).
# - 텔레그램 봇의 테이블(conversations 등)과 분리된 전용 테이블을 쓴다 (텔레그램 쪽 대화 기록을 오염시키지 않음).
# - 저장/조회 키는 웹앱이 브라우저에 보관하는 랜덤 sid뿐이다. 기록을 그대로 돌려주는 조회 API는 일부러 만들지 않았다
#   (기록은 프롬프트로만 주입됨). 인증이 없는 공개 URL이라는 점은 owner 개인화 컨텍스트와 마찬가지로 유의할 것.
...
VOICE_HISTORY_MAX_MESSAGES = _env_int("VOICE_HISTORY_MAX_MESSAGES", 30)   # 프롬프트에 싣는 최대 발화 수
VOICE_HISTORY_MAX_CHARS = _env_int("VOICE_HISTORY_MAX_CHARS", 5000)       # 프롬프트에 싣는 최대 글자 수 (프롬프트 길이/지연 방지)
VOICE_HISTORY_TTL_MIN = _env_int("VOICE_HISTORY_TTL_MIN", 20)             # 마지막 대화가 이보다 오래된 기록은 안 싣는다(분)
                                                                            # (원래 360분=6시간이었음: 이 값이 너무 크면 '네트워크가 잠깐 끊겼다 재연결된 것'과
                                                                            #  '몇 시간 뒤 다시 접속한 것'을 구분 못 하고 아침 대화를 저녁까지 그대로 이어가라고 지시하게 됨.
                                                                            #  진짜 재연결 시나리오(끊기고 몇 초~몇 분 안에 다시 붙는 것)에 맞춰 짧게 둔다.)
VOICE_LOG_RETENTION_DAYS = _env_int("VOICE_LOG_RETENTION_DAYS", 30)       # 이보다 오래된 기록은 자동 삭제
_VOICE_LOG_MAX_ROWS_PER_SID = 300
_VOICE_LOG_MAX_BATCH = 20
_VOICE_LOG_MAX_CHARS = 2000
_SID_RE = re.compile(r"^[A-Za-z0-9_-]{16,64}$")
_voice_table_ready = False
_last_voice_purge = float("-inf")


def _ensure_voice_log_table(cur) -> None:
    global _voice_table_ready
    if _voice_table_ready:
        return
    cur.execute(
        "CREATE TABLE IF NOT EXISTS voice_chat_log ("
    ...
    cur.execute("CREATE INDEX IF NOT EXISTS idx_voice_chat_log_sid ON voice_chat_log (session_key, id)")
    _voice_table_ready = True


def save_voice_log(sid: str, rows) -> int:
    """rows: [(role, text), ...] (오래된 → 최신). sid당 최근 N건만 남기고, 오래된 기록은 1시간에 한 번 정리한다."""
    global _last_voice_purge
    conn = psycopg2.connect(DATABASE_URL, sslmode="require", connect_timeout=5)
    try:
        conn.autocommit = True
        cur = conn.cursor()
        _ensure_voice_log_table(cur)
        cur.executemany(
            "INSERT INTO voice_chat_log (session_key, role, content) VALUES (%s, %s, %s)",
            [(sid, role, text) for role, text in rows],
        )
        cur.execute(
            "DELETE FROM voice_chat_log WHERE session_key = %s AND id < ("
            " SELECT id FROM voice_chat_log WHERE session_key = %s ORDER BY id DESC OFFSET %s LIMIT 1)",
            (sid, sid, _VOICE_LOG_MAX_ROWS_PER_SID - 1),
        )
        now = time.monotonic()
        if now - _last_voice_purge > 3600:
            _last_voice_purge = now
            cur.execute(
                "DELETE FROM voice_chat_log WHERE created_at < NOW() - %s * INTERVAL '1 day'",
...
def load_voice_history(sid: str):
    """[(role, content, created_at), ...] (오래된 → 최신). 테이블이 아직 없거나 실패하면 빈 리스트."""
    if not DATABASE_URL or psycopg2 is None or not _SID_RE.match(sid or ""):
        return []
    try:
        conn = psycopg2.connect(DATABASE_URL, sslmode="require", connect_timeout=5)
        try:
            ...
            cur.execute(
                ...
                " SELECT id, role, content, created_at FROM voice_chat_log"
                " WHERE session_key = %s AND created_at >= NOW() - %s * INTERVAL '1 minute'"
                " ORDER BY id DESC LIMIT %s) t ORDER BY id",
                (sid, VOICE_HISTORY_TTL_MIN, VOICE_HISTORY_MAX_MESSAGES),
            )
            rows = cur.fetchall()
            cur.close()
        finally:
        ...
        if getattr(e, "pgcode", None) != "42P01":  # 42P01: 아직 한 번도 저장된 적이 없어 테이블이 없는 경우는 정상
            logging.warning("음성 대화 기록 조회 실패 - %s", e)
        return []


def _format_elapsed_since_aware(dt) -> str:
    if dt.tzinfo is None:
        dt = dt.replace(tzinfo=datetime.timezone.utc)
    seconds = max(0.0, (datetime.datetime.now(datetime.timezone.utc) - dt).total_seconds())
    if seconds < 60:
        return "방금"
    minutes = int(seconds // 60)
    if minutes < 60:
...
def build_voice_history_context(rows):
    """직전 대화 기록을 (프롬프트 블록, 실제로 담은 발화 수)로 만든다. 글자 수 예산 안에서 최신 발화부터 채운다."""
    if not rows:
        return "", 0
    picked, used = [], 0
    for role, content, _created_at in reversed(rows):
        line = f"{'상대' if role == 'user' else '태연'}: {content}"[:VOICE_HISTORY_MAX_CHARS]
        if picked and used + len(line) > VOICE_HISTORY_MAX_CHARS:
            break
        picked.append(line)
        used += len(line)
    picked.reverse()
    elapsed_sec = max(0.0, (datetime.datetime.now(datetime.timezone.utc) - (
        rows[-1][2] if rows[-1][2].tzinfo else rows[-1][2].replace(tzinfo=datetime.timezone.utc)
    )).total_seconds())
    elapsed = _format_elapsed_since_aware(rows[-1][2])
    ...
    if elapsed_sec <= 5 * 60:
        ...
        continuity_note = (
            f"\n- 지금은 통화가 끊긴 지 {elapsed}가 지난 뒤 다시 연결된 상황이다 (방금 끊긴 게 아님). "
    ...
    block = (
        f"[직전 음성 대화 기록 - 통화 연결이 끊겼다가 다시 이어진 상황이다. 아래는 방금 전까지 실제로 나눈 대화이고 "
        f"(마지막 대화는 {elapsed}), 태연은 이 내용을 그대로 기억하고 있다]\n"
        + "\n".join(picked)
        + continuity_note
...
def issue_token():
    ...
    # ?history=1&sid=... : 이어붙일 세션 핸들 없이 '새 세션'을 시작하는 경우, 직전 대화 기록을 프롬프트에 실어 발급한다
    sid = request.args.get("sid", "")
    voice_history_ctx, history_count = "", 0
    if request.args.get("history") == "1" and _SID_RE.match(sid):
        voice_history_ctx, history_count = build_voice_history_context(load_voice_history(sid))
        ...
        config={
            ...
            "live_connect_constraints": {
                ...
                "config": {
                    ...
                    # Google 검색 그라운딩(Search grounding). gemini-3.8-live가 지원하는 서버사이드 툴이라
                    # 클라이언트(taeyeon-voice.html)가 toolCall을 받아 되돌려줄 필요 없이, 모델이 필요할 때
                    # 알아서 검색하고 답을 만들어서 내려준다. 검색 사용 지침은 VOICE_PROMPT에 있음.
                    **({"tools": [{"google_search": {}}]} if LIVE_SEARCH_ENABLED else {}),
...
def voice_log():
    """웹앱이 턴이 끝날 때마다 보내는 대화 기록 저장. body: {sid, messages: [{role: 'user'|'model', text}]}"""
    if not DATABASE_URL or psycopg2 is None:
        return jsonify({"saved": 0, "reason": "db_not_configured"})  # DB가 없으면 조용히 넘어간다 (웹앱은 재시도하지 않음)
    data = request.get_json(force=True, silent=True) or {}
    sid = str(data.get("sid", ""))
    msgs = data.get("messages")
    if not _SID_RE.match(sid) or not isinstance(msgs, list):
    ...
    try:
        return jsonify({"saved": save_voice_log(sid, rows)})
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if DATABASE_URL and psycopg2 is not None:  # 음성 대화 기록 테이블은 시작할 때 만들어 둔다 (실패해도 첫 저장 때 다시 시도)
        try:
            _c = psycopg2.connect(DATABASE_URL, sslmode="require", connect_timeout=5)
            try:
                _c.autocommit = True
                _cur = _c.cursor()
                _ensure_voice_log_table(_cur)
                _cur.close()
            finally:
                _c.close()
        except Exception as e:
            logging.warning("voice_chat_log 테이블 준비 실패(첫 저장 때 다시 시도) - %s", e)
