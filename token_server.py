...
FIND_INTERCITY_ROUTE_DECLARATION = {
    ...
    "description": (
        ...
        "알려준다(고속버스는 지금 이후 출발편의 시각과 요금도 알려준다). 사용자가 '서울 가려면 뭐 타야 돼?', '부산까지 KTX 있어?', '대구 고속버스로 얼마나 걸려?', '서울 가는 고속버스 몇 시에 있어?' 처럼 "
...
- find_intercity_route 함수도 연결되어 있다. 상대가 다른 도시(서울, 부산 같은 곳)로 기차/KTX/고속버스/시외버스로 가는 방법을 물으면 이 함수를 호출하고, 같은 도시 안에서 이동하는 질문은 find_bus_route를 쓴다. 이번 발화에서 실제로 말한 목적지가 없으면 호출하지 말고 어디 가려는지 되물을 것. 함수 결과에는 이동수단 종류, 걸리는 시간, 요금, 환승 정도가 있고, 고속버스 다음 출발편(express_bus)이 있으면 그 출발·도착 시각과 요금도 있다. express_bus에 실제로 있는 시각만 말하고(가까운 두세 개만, \"두 시 이십 분 차\"처럼 말로 풀어서), 그 밖의 출발 시각, 기차 시간표, 남은 좌석, 예매 정보는 없으니 절대 지어내지 말 것. 예약은 여기서 해줄 수 없다고 솔직하게 말하고, 정확한 좌석과 예매는 코레일톡이나 고속버스 앱 같은 데서 확인하라고만 말로 안내할 것(주소나 링크는 읽지 않는다). 가장 빠른 방법을 먼저 말하고 대안은 한 문장씩만 짧게 덧붙일 것. 분/원 숫자는 "두 시간 반쯤 걸려요"처럼 풀어서 말할 것
...
def _tago_get(operation: str, params: dict):
    ...
    for scheme in ("https", "http"):  # 명세서는 http로 안내돼 있어서, https가 안 되면 http로 재시도
        try:
            res = requests.get(f"{scheme}://{_EXPBUS_BASE}/{operation}", params=q, timeout=6)
            break
        except requests.RequestException as e:
            logging.warning("고속버스정보 호출 실패(%s, %s)", operation, type(e).__name__)
    if res is None:
        return [], None
    try:
        body = res.json()
        ...
        logging.warning("고속버스정보 응답이 JSON이 아님(%s): %s", operation, snippet)
        return [], None
    response = body.get("response") or {}
    code = str((response.get("header") or {}).get("resultCode", "00"))
    if code not in ("00", "0"):
        logging.warning("고속버스정보 결과코드 %s (%s)", code, operation)
        return [], None
...
def intercity_route():
    ...
    express_thread = None
    if EXPRESS_BUS_ENABLED:  # ODsay 조회와 동시에 돌려서 응답 시간이 늘지 않게 한다
        express_thread = threading.Thread(target=_run_express, daemon=True)
        express_thread.start()
    route = _odsay_intercity_path(lng, lat, place["lng"], place["lat"], diag)
    if express_thread:
        express_thread.join(timeout=12)
    express = express_box.get("r")
    if route is None and express:
        return jsonify({"found": True, "destination_name": place["name"],
                         "destination_address": place.get("address", ""), "express_bus": express,
                         "note": "고속버스 출발 시각만 확인됐다. express_bus에 있는 시각만 말하고, 남은 좌석/예매 정보는 없다고 안내할 것."})
    if route is None:
    ...
    if express:
        ...
        route["note"] = ("express_bus에 있는 고속버스 출발 시각만 말할 것(기차 시간표, 남은 좌석, 예매 정보는 없으니 지어내지 말 것). "
                         "예약은 못 한다고 안내할 것.")
    else:
        route["note"] = "이 결과에는 출발 시각, 남은 좌석, 예매 정보가 없다. 시각을 지어내지 말고 예약은 못 한다고 안내할 것."
        if EXPRESS_BUS_ENABLED and express_diag:
...
# 먼저 YouTube Data API로 영상 길이를 조회해서 길이에 따라 처리 방식을 나눈다:
#   - YOUTUBE_VIDEO_MAX_MINUTES(기본 30분) 이하 : Gemini에 유튜브 URL을 넘겨 영상(화면+소리)을 직접 분석
#   - 그보다 길고 YOUTUBE_MAX_MINUTES(기본 60분) 이하 : 자막(스크립트) 텍스트만 가져와서 Gemini로 요약 (토큰 절약)
#   - YOUTUBE_MAX_MINUTES 초과, 라이브/예정 영상 : 거절
...
# 자막 가져오기는 youtube-transcript-api 패키지를 쓴다(requirements.txt에 추가 필요).
# 주의: Railway 같은 클라우드 IP는 유튜브가 자막 요청을 막는 경우가 있다(RequestBlocked/IpBlocked).
...
YOUTUBE_MAX_MINUTES = _env_int("YOUTUBE_MAX_MINUTES", 60)
YOUTUBE_VIDEO_MAX_MINUTES = _env_int("YOUTUBE_VIDEO_MAX_MINUTES", 30)
_YT_TRANSCRIPT_MAX_CHARS = 120000
# (선택) 자막 요청만 프록시로 보낸다. 클라우드(Railway 등) IP는 유튜브가 막는 경우가 많아서 필요할 수 있다.
# 영상 분석(Gemini)이나 길이 조회(YouTube Data API)는 프록시를 타지 않는다. 둘 중 하나만 설정하면 된다:
#   WEBSHARE_PROXY_USERNAME / WEBSHARE_PROXY_PASSWORD : Webshare의 'Residential' 프록시 패키지 계정 (라이브러리가 공식 지원)
#   TRANSCRIPT_PROXY_URL : 그 밖의 프록시 주소. 예: http://user:pass@host:port
...
def _transcript_proxy_config():
    """설정된 프록시가 없으면 None(=프록시 없이 직접 요청)."""
    try:
        if WEBSHARE_PROXY_USERNAME and WEBSHARE_PROXY_PASSWORD:
            from youtube_transcript_api.proxies import WebshareProxyConfig
            return WebshareProxyConfig(proxy_username=WEBSHARE_PROXY_USERNAME, proxy_password=WEBSHARE_PROXY_PASSWORD)
        if TRANSCRIPT_PROXY_URL:
            from youtube_transcript_api.proxies import GenericProxyConfig
            return GenericProxyConfig(http_url=TRANSCRIPT_PROXY_URL, https_url=TRANSCRIPT_PROXY_URL)
    except Exception as e:
        logging.warning("자막 프록시 설정을 만들지 못함(%s) - 프록시 없이 진행", type(e).__name__)
    return None
...
def _fetch_youtube_transcript(video_id):
    """자막을 30초 단위 [mm:ss] 줄로 묶은 텍스트로 가져온다. 반환: ((text, is_generated), None) 또는 (None, reason).
    reason: lib_missing | no_transcript | blocked | unavailable | error"""
    try:
        from youtube_transcript_api import YouTubeTranscriptApi
        from youtube_transcript_api import _errors as yte
    except ImportError:
        return None, "lib_missing"
    try:
        tlist = YouTubeTranscriptApi(proxy_config=_transcript_proxy_config()).list(video_id)
        try:
            tr = tlist.find_transcript(["ko", "en"])
        except yte.NoTranscriptFound:
            tr = next(iter(tlist), None)   # 한국어/영어가 없으면 있는 언어 아무거나
        if tr is None:
            return None, "no_transcript"
        fetched = tr.fetch()
        is_generated = bool(getattr(tr, "is_generated", False))
    except (yte.TranscriptsDisabled, yte.NoTranscriptFound):
        return None, "no_transcript"
    except (yte.RequestBlocked, yte.IpBlocked):
        logging.warning("자막 요청이 유튜브에 의해 차단됨(서버 IP 문제일 수 있음)")
        return None, "blocked"
    except (yte.VideoUnavailable, yte.VideoUnplayable, yte.AgeRestricted):
        return None, "unavailable"
    except Exception as e:
        logging.warning("자막 가져오기 실패(%s): %s", type(e).__name__, str(e)[:200])
        return None, "error"

    lines, cur_start, buf = [], None, []

    def _flush():
        if buf:
            lines.append(f"[{cur_start // 60:02d}:{cur_start % 60:02d}] {' '.join(buf)}")

    for sn in fetched:
        t = int(sn.start)
        if cur_start is None or t - cur_start >= 30:
            _flush()
            cur_start, buf = t, []
        txt = " ".join((sn.text or "").split())
        if txt:
            buf.append(txt)
    _flush()
    text = "\n".join(lines)
    if not text:
        return None, "no_transcript"
    if len(text) > _YT_TRANSCRIPT_MAX_CHARS:
        text = text[:_YT_TRANSCRIPT_MAX_CHARS] + "\n(이후 자막은 분량 때문에 생략됨)"
    return (text, is_generated), None


def _youtube_summary_prompt(question: str, transcript_mode: bool = False) -> str:
    if transcript_mode:
        head = (
            "아래는 어떤 유튜브 영상의 자막(스크립트)이다. 자동 생성 자막이면 오인식이 있을 수 있다. "
            "이 자막만 근거로 아래 형식으로 한국어로 정리해라. 화면 내용은 알 수 없으니 추측하지 마라.\n"
        )
    else:
...
@require_google_auth
def youtube_summary():
    """body: {url, question(선택)} -> {found, video_id, mode('video'|'transcript'), minutes, title, summary, note, cached}"""
    ...
    if seconds > YOUTUBE_MAX_MINUTES * 60:
        return _fail("too_long", f"영상이 {math.ceil(seconds / 60)}분으로 너무 길어요. {YOUTUBE_MAX_MINUTES}분 이하 영상만 볼 수 있어요.")

    transcript_mode = seconds > YOUTUBE_VIDEO_MAX_MINUTES * 60
    try:
        if transcript_mode:
            got, why = _fetch_youtube_transcript(video_id)
            if not got:
                msgs = {
                    "no_transcript": "긴 영상은 자막으로만 볼 수 있는데, 이 영상엔 자막이 없어요.",
                    "blocked": "긴 영상은 자막으로만 볼 수 있는데, 유튜브가 자막 요청을 막아서 가져오지 못했어요.",
                    "unavailable": "이 영상은 자막을 가져올 수 없어요(제한된 영상일 수 있어요).",
                    "lib_missing": "자막 기능이 서버에 설치되지 않았어요.",
                }
                return _fail("transcript_" + why, msgs.get(why, "자막을 가져오지 못했어요."), 200 if why != "error" else 502)
            transcript_text, is_generated = got
            summary = _gemini_text(
                _youtube_summary_prompt(question, True)
                + "\n\n[자막 시작]\n" + transcript_text + "\n[자막 끝]"
            )
            note = ("이 영상은 길어서 화면은 못 보고 자막 텍스트로만 파악했다. 화면에 나온 내용은 모른다고 하고 지어내지 말 것."
                    + (" 자동 생성 자막이라 오인식이 있을 수 있다." if is_generated else ""))
        else:
            summary = _gemini_text(genai.types.Content(parts=[
                genai.types.Part(file_data=genai.types.FileData(file_uri=canonical)),
                genai.types.Part(text=_youtube_summary_prompt(question)),
            ]))
            note = ""
    except Exception as e:
    ...
    result = {
        "found": True, "video_id": video_id, "mode": "transcript" if transcript_mode else "video",
        "minutes": minutes, "title": info["title"], "summary": summary, "note": note,
    }
    with _yt_cache_lock:
