...
# 루트 로거 레벨을 지정해두지 않으면 기본값(WARNING)이라 logging.info(...)로 찍는 디버그
# 로그들(예: [위치 디버그])이 DEBUG_LOG_LOCATION 값과 무관하게 전혀 출력되지 않는다.
...
- ⚠️ 웃음은 절대 "하하", "크크", "킥킥", "호호", "풋" 같은 글자를 그대로 소리 내어 읽지 말 것 (그건 웃음소리가 아니라 그 낱말을 발음하는 것일 뿐임). 대신 실제로 웃는 것처럼 목소리로만 웃을 것 - 숨을 터뜨리듯, 웃음을 참듯, 코웃음 치듯 등 상황에 맞는 진짜 웃음소리를 낼 것이지 낱말을 말하지 말 것.
- ⚠️ <laugh>, <chuckle> 같은 대괄호/꺾쇠 태그는 절대 응답 텍스트에 넣지 말 것. 이 모델(Gemini Live 네이티브 오디오)은 이런 태그를 웃음소리로 바꿔주지 않고, 태그 글자 자체를 그대로 소리 내어 읽어버림. 웃음은 반드시 위 항목처럼 목소리 연기(숨소리, 터뜨리는 웃음, 킥킥거리는 톤 등)로만 표현하고, 어떤 형태의 태그/괄호 표기도 텍스트에 쓰지 말 것.
...
# ===== 신상정보 배치 대상 표시 - 텔레그램 태연봇의 PERSONAL_INFO_KEYWORDS/TAROT_FOCUS_KEYWORDS/
#       message_has_personal_info_keyword/mark_user_facts_pending와 동일하게 유지할 것 =====
# 실제 신상정보 추출(Gemini 호출 + user_facts upsert)은 텔레그램 봇 쪽 자정 배치
# (run_daily_user_facts_batch)가 하루 한 번 처리한다. 그 배치는 'user_facts_pending'에
# 표시된 유저만 처리 대상으로 삼는데, 예전엔 이 표시가 텔레그램 메시지에서만 이뤄져서
# 음성 웹앱으로만 신상정보를 얘기한 날은 그날 배치가 통째로 건너뛰어졌다. 그래서 음성 쪽
# 대화가 저장될 때도 여기서 똑같이 키워드만 로컬로(Gemini 호출 없이) 체크해서 표시해준다 -
# 실제 추출/저장 로직 자체는 여전히 텔레그램 봇 프로세스의 자정 배치가 전담한다.
PERSONAL_INFO_KEYWORDS = [
    "이름", "나이", "생일", "태어", "살아", "살고", "사는", "거주", "이사", "동네",
    "직업", "일해", "일하고", "다니는", "회사", "다녀", "출근", "퇴근", "알바",
    "학교", "학년", "전공", "대학", "졸업",
    "가족", "부모", "엄마", "아빠", "남편", "아내", "와이프", "형", "누나", "언니", "오빠",
    "동생", "딸", "아들", "결혼", "연애", "여자친구", "남자친구",
    "반려", "강아지", "고양이", "키우", "키워",
    "취미", "좋아해", "좋아하는", "싫어해", "싫어하는", "MBTI", "출신", "고향"
]
TAROT_FOCUS_KEYWORDS = [
    "고민", "고민이", "고민돼", "힘들어", "힘든", "지쳐", "지친", "스트레스", "불안해", "불안하",
    "답답해", "답답하", "진로", "이직", "취업", "시험", "면접", "짝사랑", "헤어졌", "썸",
    "고백", "좋아하는 사람"
]


def message_has_personal_info_keyword(text: str) -> bool:
    """taeyeon_bot.py의 message_has_personal_info_keyword와 동일. API 호출 없는 로컬 키워드 체크."""
    if not text:
        return False
    return any(kw in text for kw in PERSONAL_INFO_KEYWORDS) or any(kw in text for kw in TAROT_FOCUS_KEYWORDS)


def mark_user_facts_pending(user_id):
    """taeyeon_bot.py의 mark_user_facts_pending과 동일 - 오늘 자정 배치에서 신상정보를
    다시 확인해야 할 유저로 표시만 해둔다(멱등). 실패해도 음성 저장 자체는 이미 끝난 뒤라 조용히 넘어간다."""
    try:
        conn = _history_db_connect()
        try:
            cur = conn.cursor()
            cur.execute(
                """
                INSERT INTO user_facts_pending (user_id, marked_at)
                VALUES (%s, NOW())
        ...
        logging.warning("신상정보 배치 대상 표시 실패 - %s", e)
...
def save_voice_turns(rows) -> int:
    ...
    # 이번 배치에 신상정보 관련 키워드가 하나라도 있으면, 텔레그램 봇의 자정 배치가 오늘 이 유저를
    # 신상정보 재확인 대상으로 처리하도록 표시해둔다 (실제 추출은 그 배치가 밤에 한 번에 처리).
    if any(role == "user" and message_has_personal_info_keyword(text) for role, text in fresh):
        threading.Thread(target=mark_user_facts_pending, args=(OWNER_USER_ID,), daemon=True).start()
    return n
...
def build_system_prompt(stt_mode: bool = False, voice_history: str = "", location_label: str = "") -> str:
    ...
    if location_label:
        ...
        location_block = (
            ...
            "- '나 어디게', '나 어디 있게', '내가 어디 있는 것 같아?'처럼 정보를 묻는 평서문이 아니라 "
            "놀이하듯 맞혀보라는 식으로 물어보는 것도 위치를 묻는 질문으로 똑같이 취급할 것 - 위 값을 "
            "자신 있게 맞히듯 답하고, 되묻거나 딴 얘기로 넘기거나 추측하는 척하지 말 것\n"
            "- 위 위치 정보와 모순되는 걸 되묻지 말 것 (예: 이미 '집'으로 확인됐는데 \"퇴근했어?\", "
            "\"아직 회사야?\" 같은 질문 던지지 말 것)"
        )
