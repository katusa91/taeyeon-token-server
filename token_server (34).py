...
FIND_INTERCITY_ROUTE_DECLARATION = {
    ...
    "description": (
        ...
        "알려준다. 사용자가 '서울 가려면 뭐 타야 돼?', '부산까지 KTX 있어?', '대구 고속버스로 얼마나 걸려?' 처럼 "
        ...
        "'뭐 타야 돼', 'KTX 있어', '얼마나 걸려' 처럼 이동 수단·방법을 직접 물을 때만 호출할 것. "
...
- find_intercity_route 함수도 연결되어 있다. 상대가 다른 도시(서울, 부산 같은 곳)로 기차/KTX/고속버스/시외버스로 가는 방법을 물으면 이 함수를 호출하고, 같은 도시 안에서 이동하는 질문은 find_bus_route를 쓴다. 이번 발화에서 실제로 말한 목적지가 없으면 호출하지 말고 어디 가려는지 되물을 것. 함수 결과에는 이동수단 종류, 걸리는 시간, 요금, 환승 정도만 있고 출발 시각, 남은 좌석, 예매 정보는 없으니 절대 지어내지 말 것. 예약은 여기서 해줄 수 없다고 솔직하게 말하고, 정확한 시간표와 예매는 코레일톡이나 고속버스 앱 같은 데서 확인하라고만 말로 안내할 것(주소나 링크는 읽지 않는다). 가장 빠른 방법을 먼저 말하고 대안은 한 문장씩만 짧게 덧붙일 것. 분/원 숫자는 "두 시간 반쯤 걸려요"처럼 풀어서 말할 것
...
def save_voice_turns(rows) -> int:
    ...
    add_to_history가 압축까지 알아서 처리해준다. DB/owner 설정이 없으면 조용히 0건.
    저장 후 update_last_message_at으로 users.last_message_at도 갱신한다 - 이걸 안 하면 텔레그램 쪽
    reset_history_if_long_gap/get_affinity_context가 '마지막으로 대화한 시각'을 여전히 텔레그램 기준으로만
    알게 되어, 방금 음성으로 대화했는데도 며칠 전에 대화한 것처럼 취급하는 어긋남이 생긴다. owner는
    ADMIN_USER_IDS라 affinity_delta는 어차피 0으로 고정되므로 안전하다."""
    if not DATABASE_URL or psycopg2 is None or OWNER_USER_ID is None:
        return 0
    saved = 0
    for role, text in rows:
        try:
            add_to_history(OWNER_USER_ID, "assistant" if role == "model" else "user", text)
            saved += 1
        except Exception as e:
            logging.warning("음성 대화 기록 저장 실패 - %s", e)
    if saved:
        try:
            update_last_message_at(OWNER_USER_ID)
        except Exception as e:
