...
def _kakao_search_place(query: str, ref_lng: float = None, ref_lat: float = None):
    ...
    반환: {"name","address","lat","lng"} 또는 못 찾으면 None."""
    ...
    try:
        ...
        logging.warning("[버스경로] 카카오 장소 검색 실패: %s", e)
        return None
    if not docs:
        return None
    d = docs[0]
...
def bus_route():
    ...
    place = _kakao_search_place(destination, lng, lat)
    if not place:
        return jsonify({"found": False, "reason": "destination_not_found",
                         "message": f"'{destination}'라는 곳을 찾지 못했어요."})
